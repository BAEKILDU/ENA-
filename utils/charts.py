...
def _bar_labels(values: list, suffix: str = "") -> list[str]:
    return [f"{v}{suffix}" for v in values]
...
def bar_chart(
    ...
    suffix = "%" if text_template.endswith("%") else ""
    fig = go.Figure(
        go.Bar(
            x=[str(v) for v in x],
            y=y_vals,
            ...
            text=_bar_labels(y_vals, suffix),
...
def horizontal_bar_chart(
    ...
    fig = go.Figure(
        go.Bar(
            ...
            text=[f"{v}{text_suffix}" for v in vals],
            textposition="outside",
            textfont=dict(color="#e2e8f0", size=12),
